...
Lors du développement d'une application, on arrive souvent à la question de l'authentification à un autre service (API, base de données, etc).

Souvent, un des premiers réflexes est de se demander quel est le moyen le plus *sécurisé*. Et évidemment, quoi de mieux que d'utiliser la **cryptographie** pour s'authentifier ?
...
=> Il vaut mieux mettre en place une gestion des clefs dans un hardware type **HSM** comme une YubiKey ou une SmartCard.
...
=> Il faut une infrastructure **résiliente** et **sécurisée** qui permet de diffuser les CRL ou un service OCSP
...
=> Il faut une infrastructure **résiliente** et **sécurisée** qui permet de renouveler les certificats de façon régulière et automatisée.
...
* Un certificat doit certifier une identité. Croyez-le ou non, j'ai déjà rencontré des applications qui ne vérifiaient pas l'identité de l'appelant, mais **uniquement la validité du certificat** !
=> Le code de l'application doit lire l'identité de l'appelant et l'utiliser pour vérifier ses droits. Et plus globalement, *les développeurs doivent être solidement formés* à ce type d'authentification.


Maintenant, regardons rapidement les avantages d'un simple mot de passe :

* Le transit du mot de passe est **déjà sécurisé** par TLS côté *serveur* (https).
...
* La plupart des outils de gestion de secrets implémentent une **rotation automatique** des mots de passe.
* Un mot de passe peut être tout simplement *changé* en cas de compromission.
* Globalement, les développeurs maitrisent mieux les mots de passe que les certificats.
...
**Faites un choix en fonction de vos besoins, mais ne vous laissez pas tenter par la sensation de sécurité que procure l'utilisation de certificats.**